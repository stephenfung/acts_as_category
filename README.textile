...
* _Rails 2.2_ or higher (works also on _Rails 2.3_ unstable)
...
A:: Because the whole idea of hidden is to exclude descendants of an hidden Category as well, thus the ancestors of a category you can access anyway are never going to be hidden.
...
Note that there is a whole lot more options an all these gizmos, widgets and gadgets. Funkensturm provides a complete example application of a gallery using acts_as_category.
